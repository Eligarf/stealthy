...
export default class Engine {
  ...
  init() {
    ...
    game.settings.register(
      Stealthy.MODULE_ID,
      Stealthy.ALLOWED_DETECTION_MODES,
      settings.allowedDetectionModes,
    );
    game.settings.register(
      Stealthy.MODULE_ID,
      "friendlyStealth",
      settings.friendlyStealth,
    );
    ...
    game.settings.register(
      Stealthy.MODULE_ID,
      "hiddenAliases",
      settings.hiddenAliases,
    );
    game.settings.register(
      Stealthy.MODULE_ID,
      "clearAfterCombat",
      settings.clearAfterCombat,
    );

    game.settings.register(
      Stealthy.MODULE_ID,
      "stealthToActor",
      settings.stealthToActor,
    );
    game.settings.register(
      Stealthy.MODULE_ID,
      "perceptionToActor",
      settings.perceptionToActor,
    );
    game.settings.register(
      Stealthy.MODULE_ID,
      "hiddenSource",
      settings.hiddenSource,
    );
    game.settings.register(
      Stealthy.MODULE_ID,
      "hiddenIcon",
  ...
  setup() {
    this.hiddenName = game.i18n.localize(
      game.settings.get(Stealthy.MODULE_ID, "hiddenLabel"),
    );
    this.hiddenAliases = [this.hiddenName];
    this.spotName = game.i18n.localize(
      game.settings.get(Stealthy.MODULE_ID, "spotLabel"),
    );
    const aliases = game.settings.get(Stealthy.MODULE_ID, "hiddenAliases");
    if (aliases.length > 0) this.hiddenAliases.push(...aliases.split(";"));
    Stealthy.log(
  ...
  async migrate_to_4_6() {
    let modes = foundry.utils.deepClone(
      game.settings.get(Stealthy.MODULE_ID, "allowedDetectionModes"),
    );
    let changed = false;
  ...
  async ready() {
    ...
    let allowedModes = game.settings.get(
      Stealthy.MODULE_ID,
      Stealthy.ALLOWED_DETECTION_MODES,
    );
  ...
  checkDispositionAndCanDetect(
    ...
    if (tgtToken?.disposition === visionSource.object.document?.disposition) {
      const friendlyStealth = game.settings.get(
        Stealthy.MODULE_ID,
        "friendlyStealth",
      );
      if (
        friendlyStealth === "ignore" ||
        (!game.combat && friendlyStealth === "inCombat")
      )
        return true;
    }
    ...
    // Otherwise, if we are token based then we need to update the token value
      ...
      if (value === undefined) {
        if (typeof _del !== "undefined")
          update[`flags.stealthy.${skill}`] = _del;
        else update[`flags.stealthy.-=${skill}`] = null;
      } else {
  ...
  async bankPerception(token, value) {
    ...
    await this.updateOrCreatePerceptionEffect(token.actor, {
      perception: value,
    });
  }
  ...
  async createClearMessage(token, skill, value) {
    ...
    const tooltip = game.i18n.localize(
      `${Stealthy.MODULE_ID}.clearAfterCombat.restoreBank`,
    );
    const content = `
    <div>
      <span>${text}</span>
  ...
  async clearStealth(token) {
    ...
    if (stealthy.stealthToActor) {
      ...
      if (value === undefined) return;
      let update = { _id: token.id };
      if (typeof _del !== "undefined") update[`flags.stealthy.stealth`] = _del;
      else update["flags.stealthy.-=stealth"] = null;
      await canvas.scene.updateEmbeddedDocuments("Token", [update]);
    }
  ...
  async clearPerception(token) {
    ...
    if (stealthy.perceptionToActor) {
      ...
      let update = { _id: token.id };
      if (typeof _del !== "undefined")
        update[`flags.stealthy.perception`] = _del;
    ...
    switch (source) {
      case "ce": {
        ...
        if (!effect && !this.warnedMissingCE) {
          ...
          console.error(
            `stealthy | Convenient Effects couldn't find the '${name}' effect so Stealthy will use the default one. Add your customized effect to CE or select a different effect source in Game Settings`,
          );
        }
        break;
      }
      ...
      case "clt": {
        ...
        if (!effect && !this.warnedMissingCLT) {
          ...
          console.error(
            `stealthy | Condition Lab & Triggler couldn't find the '${name}' effect so Stealthy will use the default one. Add your customized effect to CLT or select a different effect source in Game Settings`,
          );
        }
        break;
      }
  ...
  async updateOrCreateEffect({
    ...
    if (!effect) {
      effect = await this.createSourcedEffect({
        name,
        actor,
        source,
        makeEffect,
  ...
  getLightExposure(token) {
    ...
    if (exposure !== "dark") {
      const lights = scene.lights
        ...
        .concat(
          scene.tokens
            .filter((t) => t.object?.light?.active)
            .map((t) => t.object.light),
        )
        .filter(
          (light) =>
            light instanceof foundry.canvas.sources.PointDarknessSource,
        )
    ...
    const bright = lights.find(
      (light) =>
        scale * canvas.grid.measurePath([center, light]).distance <
        light.data.bright,
    );
