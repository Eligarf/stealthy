...
class Engine5e extends Engine {
  ...
  setup() {
    ...
    Hooks.on("renderSettingsConfig", (app, html, data) => {
      $("<div>")
        ...
        .insertBefore(
          $('[name="stealthy.perceptionDisadvantage"]').parents(
  ...
  getSettingsParameters(version) {
    ...
    if (hidingAvailable) {
      ...
      settings.hiddenIcon.default =
        "systems/dnd5e/icons/svg/statuses/hiding.svg";
  ...
  canDetect({
    ...
    const perceptionValue = game.settings.get(
      Stealthy.MODULE_ID,
      "perceptionDisadvantage",
    )
      ? this.adjustForLightingConditions({
          perceptionPair,
          visionSource,
          source,
          tgtToken,
      ...
      : this.adjustForDefaultConditions({
          perceptionPair,
          visionSource,
          source,
          tgtToken,
    ...
    Stealthy.logIfDebug(
      `${detectionMode} vs '${tgtToken.name}': ${perceptionValue} vs ${stealthValue}`,
      { stealthFlag, perceptionFlag },
    );
  ...
  adjustForDefaultConditions({ perceptionPair, source }) {
    const passivePrc =
      source?.system?.skills?.[
        game.settings.get(Stealthy.MODULE_ID, "perceptionKey")
      ]?.passive ?? -100;
    let perception = perceptionPair?.normal ?? perceptionPair ?? passivePrc + 1;
    return perception;
  }
  ...
  adjustForLightingConditions({
    ...
    switch (detectionMode) {
      case "basicSight":
        ...
        if (visionSource.radius > 0) lightBand += 1;
        break;
      case "devilsSight":
    ...
    if (oldBand != lightBand)
      Stealthy.logIfDebug(
        `${detectionMode} vs '${tgtToken.name}': ${Engine5e.LIGHT_LABELS[oldBand]}-->${Engine5e.LIGHT_LABELS[lightBand]}`,
      );
  ...
  getStealthFlag(token) {
    ...
    if (flag && flag.stealth === undefined)
      flag.stealth =
        flag.token.actor.system?.skills?.[
          game.settings.get(Stealthy.MODULE_ID, "stealthKey")
        ]?.passive ?? -100;
    return flag;
  }
  ...
  getPerceptionFlag(token) {
    ...
    const disadvantagedPassive =
      token.actor?.flags?.["midi-qol"]?.disadvantage?.skill?.[prcKey] > 0
        ? passive
        : passive - 5;
    return {
      token,
  ...
  async setPerceptionValue(flag, value) {
    if (value === undefined) await super.setPerceptionValue(flag, value);
    else {
  ...
  async createSourcedEffect({ name, actor, source, makeEffect }) {
    ...
    if (effect) {
      ...
      console.error(
        `stealthy | Chris's Premades couldn't find the '${name}' effect so Stealthy will use the default one. Add your customized effect to CPR or select a different effect source in Game Settings`,
      );
    }
    return effect;
  }
  ...
  async rollPerception(actor, roll) {
    ...
    if (
      ...
      game.settings.get(Stealthy.MODULE_ID, "perceptionDisadvantage")
    ) {
    ...
    if (!game.settings.get(Stealthy.MODULE_ID, "ignorePassiveFloor")) {
      ...
      perception.disadvantaged = Math.max(
        ...
        actor.flags?.["midi-qol"]?.disadvantage?.skill?.[prcKey] > 0
          ? passivePrc
          : passivePrc - 5,
      );
    }
