...
export class DetectionModesApplicationClass extends HandlebarsApplicationMixin(
  ApplicationV2,
) {
  ...
  get #detectionModes() {
    return foundry.utils.deepClone(
      game.settings.get(Stealthy.MODULE_ID, Stealthy.ALLOWED_DETECTION_MODES),
    );
  }
  ...
  async _prepareContext() {
    ...
    const entries = Object.entries(this.#detectionModes)
      .filter(
        ([k, v]) => k in CONFIG.Canvas.detectionModes && k !== "undefined",
      )
      .map(([k, v]) => [
        k,
        {
      ...
      .sort((a, b) =>
        game.i18n
          .localize(a[1].label)
          .localeCompare(game.i18n.localize(b[1].label)),
      );
    context.detectionModes = Object.fromEntries(entries);
    if ("lightBased" in entries[0][1]) context.lightBased = true;
    return context;
  }

  static async #onSubmit(event, form, formData) {
    const object = formData.object;
    const original = game.settings.get(
      Stealthy.MODULE_ID,
    ...
    if (JSON.stringify(object) !== JSON.stringify(original)) {
      ui.notifications.warn(
        game.i18n.localize(`${Stealthy.MODULE_ID}.detectionModesMenu.warning`),
      );
